...
mcpup API, writes it to disk recursively, runs `npm install`, `npm run build`,
then `npm run start`.
...
| `MCPUP_API_TOKEN`     | no       | —                                  | Bearer token sent with the archive request.        |
...
```bash
docker run --rm osamanj93/mcpup-bootstrap npm run typecheck
```
